...
\begin{varbox}
\begin{align*}
  m_i &\geq 0,\quad i \in \cset{I} &&\text{libras de MP destinadas al insumo }i\\
  p_j &\geq 0,\quad j \in \cset{J} &&\text{ejecuciones del proceso }j\\
  y_k &\geq 0,\quad k \in \{C,D\}  &&\text{oz del producto }k\text{ producidas}\\
  w   &\geq 0 &&\text{oz de desecho derramado al río}
\end{align*}
\end{varbox}
...
\begin{conjbox}
\begin{align*}
  \cset{M} &= \{1,\,2,\,3\} &&\text{métodos de procesamiento de crudo}\\
  \cset{G} &= \{6,\,8,\,10\} &&\text{grados de crudo}\\
  \cset{P} &= \{\text{gas},\,\text{oil}\} &&\text{productos finales (gasolina, fuel oil)}
\end{align*}
\end{conjbox}
...
\toprule
\textbf{Símbolo} & \textbf{Descripción} & \textbf{Mét.\ 1} & \textbf{Mét.\ 2} & \textbf{Mét.\ 3} & \textbf{Unidad}\\
\midrule
$r_{g,m}$  & Rendimiento grado $g$ por barril & \text{ver tabla} & & & bbl/bbl\\
$k_m$      & Costo de procesamiento          & 3.4 & 3.0 & 2.6 & USD/bbl\\
\bottomrule
\end{tabular}}

\medskip
\textbf{Rendimiento por método} $r_{g,m}$:
\begin{center}
\begin{tabular}{@{}lccc@{}}
\toprule
...
\begin{itemize}[leftmargin=1.5em]
  \item Cracking catalítico: grado $6 \to 8$: \$1.30/bbl;\quad grado $8 \to 10$: \$2.00/bbl
  \item Desecho de excedente: \$0.20/bbl
  \item Gasolina: venta \$8/bbl, grado mín.\ 9, máx.\ 2\,000 bbl
  \item Fuel oil: venta \$6/bbl, grado mín.\ 7, máx.\ 600 bbl
\end{itemize}
\end{parambox}
...
\begin{varbox}
\begin{align*}
  x_m     &\geq 0,\quad m \in \cset{M}   && \text{barriles procesados por método $m$}\\
  f_6     &\geq 0                         && \text{barriles grado 6 crackeados a grado 8}\\
  f_8     &\geq 0                         && \text{barriles grado 8 crackeados a grado 10}\\
  g_k     &\geq 0,\quad k \in \cset{G}   && \text{barriles grado $k$ destinados a gasolina}\\
  o_k     &\geq 0,\quad k \in \cset{G}   && \text{barriles grado $k$ destinados a fuel oil}\\
  d_k     &\geq 0,\quad k \in \cset{G}   && \text{barriles grado $k$ descartados}
\end{align*}
\end{varbox}
...
\begin{conjbox}
\begin{align*}
  \cset{T} &= \{1,\,2,\,3,\,4\} &&\text{trimestres del año}\\
  \cset{S} &= \{1,\,2,\,3,\,4\} &&\text{tipos de empleado (libre en trimestre }s\text{)}
\end{align*}
\end{conjbox}
...
\midrule
$d_t$  & Demanda de licuadoras en el trimestre $t$ & $d_1=4000,\,d_2=2000,\,d_3=3000,\,d_4=10000$ & ud\\
$\text{sal}$ & Salario anual por empleado & 30\,000 & \$/emp.\\
$h$    & Costo unitario de inventario & 30 & \$/ud/trim.\\
$\text{cap}$ & Producción máx.\ por empleado activo & 500 & ud/trim.\\
$I_0$  & Inventario inicial                  & 600 & ud\\
$\delta_{st}$ & Indicador: empleado tipo $s$ trabaja en $t$ & $\delta_{st}=1$ si $s\neq t$, 0 si $s=t$ & —\\
\bottomrule
\end{tabular}}
\end{parambox}
...
\begin{varbox}
\begin{align*}
  w_s &\geq 0,\quad s \in \cset{S} &&\text{empleados con trimestre libre }s\\
  P_t &\geq 0,\quad t \in \cset{T} &&\text{licuadoras producidas en trimestre }t\\
  I_t &\geq 0,\quad t \in \cset{T} &&\text{inventario al final del trimestre }t
\end{align*}
\end{varbox}
...
\begin{conjbox}
\begin{align*}
  \cset{T} &= \{d,\,n\} &&\text{tipo de llamada (diurna, nocturna)}\\
  \cset{G} &= \{\text{ES},\,\text{EO},\,\text{VS},\,\text{MS}\}
            &&\text{grupos: esposa, esposo, varón soltero, mujer soltera}
\end{align*}
\end{conjbox}
...
\textbf{Símbolo} & \textbf{Descripción} & \textbf{Valor} & \textbf{Unidad}\\
\midrule
$c_t$      & Costo por llamada tipo $t$   & $c_d=2,\;c_n=5$ & \$/llamada\\
$\alpha_{gt}$ & Fracción de llamadas tipo $t$ que alcanzan al grupo $g$ &
  ver tabla & —\\
$D_g$      & Detecciones mínimas del grupo $g$ &
  $D_{ES}=150,\;D_{EO}=120,\;D_{VS}=100,\;D_{MS}=110$ & personas\\
\bottomrule
\end{tabular}}
...
\begin{varbox}
\[
  x_t \;\geq 0, \quad t \in \cset{T}
\]
$x_t$ representa el número de llamadas de tipo $t$ realizadas.
\end{varbox}
...
\[
  \cset{F} = \{1,\,2,\,T\}
  \quad\text{fuentes de madera: grado 1, grado 2 y troncos propios}
\]
\end{conjbox}
...
\begin{tabular}{@{}clcccc@{}}
\toprule
\textbf{Símbolo} & \textbf{Descripción} & \textbf{Grado 1} & \textbf{Grado 2} & \textbf{Troncos} & \textbf{Unidad}\\
\midrule
$c_f$       & Costo total por pie$^3$ de entrada & 7 & 11 & 9.5 & \$/pie$^3$\\
$r_f$       & Rendimiento de madera útil          & 0.70 & 0.90 & 0.80 & pie$^3$/pie$^3$\\
$\tau_f$    & Tiempo de secado por pie$^3$        & 1.2 & 0.8 & 1.3 & s/pie$^3$\\
$\sigma_f$  & Capacidad de oferta                 & 40\,000 & 60\,000 & 35\,000 & pie$^3$/sem.\\
\midrule
$D$    & Demanda semanal de tablones útiles & \multicolumn{3}{c}{90\,000} & pie$^3$/sem.\\
$T_{\max}$ & Tiempo de secado disponible    & \multicolumn{3}{c}{144\,000} & s/sem.\\
\bottomrule
\end{tabular}}

\smallskip
\textit{Nota: $c_T = 2.5\text{ (flete)} + 3\text{ (corte)} + 4\text{ (secado)} = 9.5$ \$/pie$^3$}
\end{parambox}
...
\begin{varbox}
\[
  x_f \;\geq 0, \quad f \in \cset{F}
\]
$x_f$ representa los pies$^3$ de madera adquiridos / enviados al aserradero desde la fuente $f$.
\end{varbox}
...
\begin{conjbox}
\begin{align*}
  \cset{M} &= \{A,\,B\} &&\text{tipos de chatarra}\\
  \cset{C} &= \{\text{Al},\,\text{Si},\,\text{Ca}\} &&\text{componentes de la aleación (aluminio, silicio, carbón)}
\end{align*}
\end{conjbox}
...
\begin{tabular}{@{}clcc@{}}
\toprule
\textbf{Símbolo} & \textbf{Descripción} & \textbf{Chatarra A} & \textbf{Chatarra B}\\
\midrule
$c_m$         & Costo por tonelada [\$/t]    & 100 & 80\\
$a_{\text{Al},m}$ & Fracción de aluminio     & 0.06 & 0.03\\
$a_{\text{Si},m}$ & Fracción de silicio      & 0.03 & 0.06\\
$a_{\text{Ca},m}$ & Fracción de carbón       & 0.04 & 0.03\\
\midrule
\multicolumn{4}{@{}l}{$Q = 1\,000$ t: producción total requerida}\\
\end{tabular}
\medskip

\textbf{Límites de composición:}
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Componente $k$} & \textbf{Mín (\%)} & \textbf{Máx (\%)} & $\underline{q}_k / \overline{q}_k$ (en t)\\
\midrule
Aluminio (Al) & 3 & 6 & 30 / 60\\
Silicio  (Si) & 3 & 5 & 30 / 50\\
Carbón   (Ca) & 3 & 7 & 30 / 70\\
\bottomrule
\end{tabular}}
\end{parambox}
...
\begin{varbox}
\[
  x_m \;\geq 0, \quad m \in \cset{M}
\]
$x_m$ representa las toneladas de chatarra $m$ utilizadas en la mezcla.
\end{varbox}
